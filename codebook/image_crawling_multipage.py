...
def crawl_megacoffee_menu(page,name):
    # Selenium 웹드라이버 설정
    ...
    while True:
        print(f"현재 페이지: {current_page}")  # 현재 페이지 번호 출력
        soup = BeautifulSoup(driver.page_source, 'html.parser')
        menu_items = soup.select('#menu_list li')
        ...
        for item in menu_items:
            ...
            safe_menu_name = menu_name.replace('/', '-').replace('\\', '-')  # 파일 이름으로 사용할 수 없는 문자 대체
            image_filename = os.path.join('images', safe_menu_name + '.jpg')
            download_image(image_url, image_filename)

            # 메뉴 정보를 딕셔너리에 저장
            menu_dict[menu_name] = menu_description
        ...
        try:
            current_page += 1
            next_page_selector = f'.board_page_link[data-page="{current_page}"]'
            next_page_button = driver.find_element(By.CSS_SELECTOR, next_page_selector)
            next_page_button.click()
            driver.implicitly_wait(10)
        except NoSuchElementException:
            print(f"페이지 {current_page}가 없음, 크롤링 종료")
            break
    driver.quit()
    print(menu_dict)
    txt_name = name + '.txt'
    with open(txt_name, 'w') as file:
        file.write(str(menu_dict))
    return menu_dict        
# 크롤링 실행
# crawl_megacoffee_menu('https://www.mega-mgccoffee.com/menu/?menu_category1=1&menu_category2=1','Beverages')
# crawl_megacoffee_menu('https://www.mega-mgccoffee.com/menu/?menu_category1=2&menu_category2=2','food')
crawl_megacoffee_menu('https://www.mega-mgccoffee.com/menu/?menu_category1=3&menu_category2=3','product')
